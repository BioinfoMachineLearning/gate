...
if __name__ == '__main__':
    ...
    for fold in range(10):
        ...
        cmd = f"python gate/network/edge_directed_kmeans_node_pairwise_loss/train_single_fold_seed_v5_af_mango.py "\
            f"--datadir dataset/CASP15_inhouse_full/sample/kmeans_v7/kmeans_sil_t3000/ " \
            f"--scoredir dataset/CASP15_inhouse_full/ " \
            f"--outdir dataset/CASP15_inhouse_full/processed_dataset_directed_kmeans_v7/{args.threshold}/kmeans_sil_t3000/ " \
            f"--project directed_kmeans_af_v8_t3000_sim{args.threshold} " \
            f"--dbdir experiments/ " \
            f"--labeldir dataset/CASP15_inhouse_full/label/ " \
            f"--log_val_mse true " \
            f"--fold {fold}"

        jobname = f"directed_kmeans_af_v8_t3000_sim{args.threshold}_fold{fold}"
