...
if __name__ == '__main__':
    ...
    parser.add_argument('--indirs', type=str, required=True)
    ...
    for indir in os.listdir(args.indirs):
        ...
        for i in range(len(os.listdir(args.nativedir))):
            ...
            spear_corrs_print += [float(group_res[args.field]['spear_corrs'][i])]

            contents += [group_res[args.field]['losses'][i]]
            losses_print += [float(group_res[args.field]['losses'][i])]

            contents += [group_res[args.field]['MSEs'][i]]
            mses_print += [float(group_res[args.field]['MSEs'][i])]

            #print(' '.join(contents))
        # print(losses_print)
        print(indir)
        print(f"Correlation\tSpear Correlation\tRanking loss\tMSE")
        print(f"{np.mean(np.array(corrs_print))}\t{np.mean(np.array(spear_corrs_print))}\t{np.mean(np.array(losses_print))}\t{np.mean(np.array(mses_print))}")
