...
if __name__ == '__main__':
    ...
    QA_scores = ['alphafold', 'contact', 'dproqa', 'pairwise', 'pairwise_usalign', 
                 'pairwise_qsscore', 'voro_scores', 'enqa_interface', 
                 'gcpnet_interface']
    ...
    for target in targets:
        ...
        for QA_score in QA_scores:
            if QA_score == 'gcpnet_interface':
                for ema_score in ['esm_plddt_pred_pdbs', 'no_esm_pred_pdbs', 'no_plddt_pred_pdbs']:
                    csv_file = f"{args.indir}/{QA_score}/{ema_score}/{target}.csv"
                    df = pd.read_csv(csv_file)
                    for i in range(len(df)):
                        model = df.loc[i, 'model']
                        score = df.loc[i, 'iplddt']
                        score_norm = df.loc[i, 'iplddt']
                        if ema_score == 'esm_plddt_pred_pdbs':
                            gcpnet_esm_plddt_dict[model] = score
                            gcpnet_esm_plddt_norm_dict[model] = score_norm
                        elif ema_score == 'no_esm_pred_pdbs':
                            gcpnet_noesm_dict[model] = score
                            gcpnet_noesm_norm_dict[model] = score_norm
                        elif ema_score == 'no_plddt_pred_pdbs':
