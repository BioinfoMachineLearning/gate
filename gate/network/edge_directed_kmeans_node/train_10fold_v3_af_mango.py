...
def objective_graph_transformer(random_seed, projectname, workdir, outdir, num_heads, num_layer, dp_rate, hidden_dim, 
                                mlp_dp_rate, loss_fun, lr, weight_decay, layer_norm,
                                batch_size, ckpt_root_dir, log_train_mse, log_val_mse):
    ...
    dgldir = f"{outdir}/processed_data/dgl"
    labeldir = f"{outdir}/processed_data/label"

    early_stop = False
    fold_valid_loss = 0
    for fold in range(10):

        folddir = f"{outdir}/fold{fold}"
        ...
        fold_ckpt_dir = ckpt_dir + '/fold' + fold
        os.makedirs(fold_ckpt_dir, exist_ok=True)

        run_json_file = fold_ckpt_dir + '/learning_curve.json'
        ...
        if not os.path.exists(run_json_file):
            ...
            model_dict = {}
            with open(fold_ckpt_dir + '/config.json', 'w') as fw:
        ...
        fold_valid_loss += np.min(valid_loss[1:])
    
    penalty = 0
    if early_stop:
        penalty = 10

    loss = fold_valid_loss / 10
...
def cli_main():
    ...
    workdir = f"{args.dbdir}/{args.project}"
