...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, dp_rate=0.5, L=2):
        ...
        for i in range(L):
            self.layers1.add_module(f'Linear {i}', nn.Linear(input_dim // 2 ** i, input_dim // 2 ** (i + 1), bias=True))
            self.layers1.add_module(f'BN {i}', nn.BatchNorm1d(input_dim // 2 ** (i + 1)))
            self.layers1.add_module(f'relu {i}', nn.LeakyReLU())
            self.layers1.add_module(f'dp {i}', nn.Dropout(p=dp_rate))
        self.final_layer1 = nn.Linear(input_dim // 2 ** L, output_dim, bias=True)

        self.layers2 = nn.Sequential()
        for i in range(L):
            self.layers2.add_module(f'Linear {i}', nn.Linear(input_dim // 2 ** i, input_dim // 2 ** (i + 1), bias=True))
            self.layers2.add_module(f'BN {i}', nn.BatchNorm1d(input_dim // 2 ** (i + 1)))
            self.layers2.add_module(f'relu {i}', nn.LeakyReLU())
            self.layers2.add_module(f'dp {i}', nn.Dropout(p=dp_rate))
        self.final_layer2 = nn.Linear(input_dim // 2 ** L, output_dim, bias=True)

    def forward(self, x):
        x1 = self.layers1(x)
        y1 = torch.sigmoid(self.final_layer1(x1))  # dockq_score

        x2 = self.layers2(x)
        y2 = torch.sigmoid(self.final_layer2(x2))  # dockq_score
        return y1, y2
