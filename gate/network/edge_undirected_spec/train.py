...
def cli_main():
    ...
    for sampled_data in os.listdir(args.datadir):
        ...
        for i, (train_val_index, test_index) in enumerate(kf.split(all_targets)):
            ...
            workdir = sampled_data + '_undirected'
            os.makedirs(sampled_data + '_undirected', exist_ok=True)
                        ...
                        for layer_norm in [True]:
                            for hidden_dim in [16]:
                                for mlp_dp_rate in [0.3, 0.5]:
                                    ...
                                    trainer = L.Trainer(accelerator='gpu',max_epochs=300, logger=wandb_logger)

                                    wandb_logger.watch(model)

                                    trainer.fit(model, train_loader, val_loader)
                                    
                                    os.system(f'touch {workdir}/{num_heads}_{num_layer}_{dp_rate}_{layer_norm}_{hidden_dim}_{mlp_dp_rate}.done')
