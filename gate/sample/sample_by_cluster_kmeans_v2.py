...
def sample_models_by_kmeans(indir, sample_number_per_target, outdir_base, minclust):
    ...
    for pairwise_file in os.listdir(indir):
        ...
        kmeans_cluster_num = minclust + np.argmax(silhouette_scores)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--minclust', type=int, required=True)
    ...
    for sample_number_per_target in sample_numbers_per_target:
        ...
        makedir_if_not_exists(outdir)
        sample_models_by_kmeans(args.indir, sample_number_per_target, outdir, args.minclust)
