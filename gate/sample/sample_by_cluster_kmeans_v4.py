...
def sample_models_by_kmeans(mmaligndir, qsscoredir, sample_number_per_target, outdir_base):

    for pairwise_file in os.listdir(mmaligndir):
        ...
        mean_pairwise_score = np.mean(np.array(pairwise_mmalign_graph))
        ...
        if mean_pairwise_score < 0.8:
            ...
            silhouette_scores = []
            for k in range(2, 10):
                kmeans =  KMeans(n_clusters=k, random_state=0, n_init="auto").fit(pairwise_mmalign_graph_np)
                cluster_labels = kmeans.fit_predict(pairwise_mmalign_graph_np)
                silhouette_scores.append(silhouette_score(pairwise_mmalign_graph_np, cluster_labels))

            kmeans_cluster_num = 2 + np.argmax(silhouette_scores)
            sample_number_in_cluster = int(50 / kmeans_cluster_num)

            kmeans = KMeans(n_clusters=kmeans_cluster_num, random_state=0, n_init="auto").fit(pairwise_mmalign_graph)
            ...
            for i in range(sample_number_per_target):
                ...
                selected_columns = [pairwise_mmalign_graph.columns[i] for i in subgraph_indices]
            ...
            for i in range(sample_number_per_target):
                subgraph_indices = []
                if len(pairwise_mmalign_graph.columns) >= sample_number_in_cluster:
                    subgraph_indices = random.sample(range(len(pairwise_mmalign_graph.columns)), k=sample_number_in_cluster) 
                else:
                    subgraph_indices = range(len(pairwise_mmalign_graph.columns))
...
if __name__ == '__main__':
    ...
    for sample_number_per_target in sample_numbers_per_target:
        ...
        makedir_if_not_exists(outdir)
        sample_models_by_kmeans(args.mmaligndir, args.qsscoredir, sample_number_per_target, outdir)
